...
json_file = '/home/etri/ByteTrack/datasets/DanceTrack/annotations/train.json'
coco = COCO(json_file)
ids = coco.getImgIds()
...
for ids_ in ids:
    ...
    if ii%1==0:
        ...
        tmp_img = draw_img*0.6+tmp_img*0.4
...
json_file = '/home/etri/ByteTrack/datasets/DanceTrack/annotations/val.json'
coco = COCO(json_file)
ids = coco.getImgIds()
...
for ids_ in ids:
    ...
    if ii%1==0:
        ...
        anno_ids = coco.getAnnIds(imgIds=[ids_], iscrowd=False)
        annotations = coco.loadAnns(anno_ids)
        img_file = '/home/etri/ByteTrack/datasets/DanceTrack/val/'+anno[0]['file_name']
        print(img_file)
        save_path = img_file.replace('img1','instance').replace('.jpg','.png').replace('crowdhuman_','crowdhuman_instance_')
        img = cv2.imread(img_file)        
        img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
