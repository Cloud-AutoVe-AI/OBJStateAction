...
def my_filter_detections(args, all_scores, decoded_boxes_batch):
    scores, inds = all_scores.max(1)
    c_mask = scores.gt(args.CONF_THRESH)  # greater than minmum threshold
    scores = scores[c_mask].squeeze()
    inds = inds[c_mask].squeeze()
    ...
    inds = inds[ids[:min(args.TOPK,counts)]].cpu().numpy()
