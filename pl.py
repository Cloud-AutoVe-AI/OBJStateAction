...
class DataModuleRoad(pl.LightningDataModule):
    ...
    def val_dataloader(self):
        ...
        training= False
        self.val_dataset = datasets.ROADDataset(train=training,transform=self.val_transform)
        print("Validation Set : ", len(self.val_dataset))
        self.idx+=1
        print("Validation idx : ",self.idx)
        
        val_data_loader = data_utils.DataLoader(self.val_dataset, batch_size=BATCH_SIZE, shuffle=False, num_workers=8,collate_fn=custum_collate, pin_memory=True, drop_last=True)
...
class Model(pl.LightningModule):
    def __init__(self):
        ...
        ckpt = torch.load('/data/road-dataset/My_YOLOX/yolox_m.pth.tar')
        model = load_my_state_dict(model,ckpt["model"])
        #model = load_my_state_dict(model, torch.load(args.TRAINED_PATH))
        self.base = model
...
checkpoint_callback = ModelCheckpoint(save_top_k=5,monitor='mAP',mode='max',filename='{epoch:02d}_{mAP:.5f}')
checkpoint_callback2 = ModelCheckpoint(save_last=True,every_n_epochs=10,filename='{epoch:02d}')
...
trainer = pl.Trainer(gpus=[0,1,2,3],accelerator='ddp',plugins=DDPPlugin(find_unused_parameters=False),sync_batchnorm=True,logger=tb_logger,log_every_n_steps=2,reload_dataloaders_every_n_epochs=1, max_epochs=650,callbacks=[lr_monitor, checkpoint_callback, checkpoint_callback2])
model = Model()
